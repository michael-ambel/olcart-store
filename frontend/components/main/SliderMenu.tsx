...
const SliderMenu: React.FC<ProfileSidebarProps> = ({
  isOpen,
  onClose,
  logout,
}) => {
  ...
  return (
    <div
      ref={sidebarRef}
      className={`slider-menu-container fixed h-auto top-[140px] xl:top-[130px] right-0 w-[280px] bg-white/95 shadow-2xl rounded-lg transform transition-all ease-[cubic-bezier(0.4,0,0.2,1)] ${
        isOpen
          ? "translate-x-0 opacity-100 visible duration-1000"
          : "translate-x-full opacity-0 invisible duration-500"
      } p-4 flex flex-col gap-3 z-40 border border-mo/10 backdrop-blur-lg overflow-y-auto`}
    >
      {/* Menu Items */}
      <div className="flex flex-col gap-[26px] pb-4">
        <Link
          ...
          onClick={onClose} // Close the slider when this link is clicked
        >
          <User
            size={26}
